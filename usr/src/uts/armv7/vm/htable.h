...
struct hat_cpu_info {
	...
	arm_lpte_t *hci_vlp_l3ptes;	/* VLP Level==3 pagetable (top) */
	arm_lpte_t *hci_vlp_l2ptes;	/* VLP Level==2 pagetable */
#endif	/* __amd64 */
};
 ...
 */
extern htable_t *htable_getpte(struct hat *, uintptr_t, uint_t *, arm_lpte_t *,
	level_t);
 ...
 *
 */
extern arm_lpte_t htable_walk(struct hat *hat, htable_t **ht, uintptr_t *va,
	uintptr_t eaddr);
 ...
 *
 * Note that all accesses except arm_pte_copy() and arm_pte_zero() are atomic.
 */
extern void	arm_pte_cpu_init(cpu_t *);
extern void	arm_pte_cpu_fini(cpu_t *);

extern arm_lpte_t	arm_pte_get(htable_t *, uint_t entry);

/*
 * arm_pte_set returns LPAGE_ERROR if it's asked to overwrite a page table
 * link with a large page mapping.
 */
#define	LPAGE_ERROR (-(arm_lpte_t)1)
extern arm_lpte_t	arm_pte_set(htable_t *, uint_t entry, arm_lpte_t new, void *);

extern arm_lpte_t arm_pte_inval(htable_t *ht, uint_t entry,
	arm_lpte_t old, arm_lpte_t *ptr, boolean_t tlb);

extern arm_lpte_t arm_pte_update(htable_t *ht, uint_t entry,
	arm_lpte_t old, arm_lpte_t new);

extern void	arm_pte_copy(htable_t *src, htable_t *dest, uint_t entry,
	uint_t cnt);

/*
 * access to a pagetable knowing only the pfn
 */
extern arm_lpte_t *arm_pte_mapin(pfn_t, uint_t, htable_t *);
extern void arm_pte_mapout(void);
