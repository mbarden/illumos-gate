...
#define	PTE_CLR(p, f)	((p) &= ~(arm_lpte_t)(f))
...
struct htable;
struct hat_mmu_info {
	arm_lpte_t pt_nx;		/* either 0 or PT_NX */
	arm_lpte_t pt_global;	/* either 0 or PT_GLOBAL */
	...
	arm_lpte_t *kmap_ptes;	/* mapping of pagetables that map kmap */
	...
	arm_lpte_t ptp_bits[MAX_NUM_LEVEL];	/* bits set for interior PTP */
	arm_lpte_t pte_bits[MAX_NUM_LEVEL];	/* bits set for leaf PTE */
...
#define	GET_PTE(ptr)		(*(arm_lpte_t *)(ptr))
#define	SET_PTE(ptr, pte)	(*(arm_lpte_t *)(ptr) = pte)
...
extern arm_lpte_t get_pte64(arm_lpte_t *ptr);
...
#define	PT_INDEX_PTR(p, x) \
	((arm_lpte_t *)((uintptr_t)(p) + ((x) << mmu.pte_size_shift)))
...
#ifdef __xpv
extern pfn_t pte2pfn(arm_lpte_t, level_t);
#endif
